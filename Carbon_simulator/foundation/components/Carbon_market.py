...
class Carbon_auction(BaseComponent):
    ...
    def __init__(
        ...
        # The max amount (in coin) that an agent can bid/ask for 1.0.0 unit of a commodity
    ...
    # --------------------------------------------
    def remove_orders(self, resource):
        """
        Increment the time counter for any unfilled bids/asks and remove expired
        orders from the market.
        ...
        for resource in self.commodities:
            ...
            asks_ = []
            for ask in self.asks[resource]:
                ### DO NOT Return the set aside resource to the seller ###
                '''resource_unit = world.agents[ask["seller"]].escrow_to_inventory(
                    resource, 1
                )
                assert resource_unit == 1'''
    ...
    def get_n_actions(self, agent_cls_name):
        ...
        Adds 2*C action spaces [ (bid+ask) * n_commodities ], each with 1.0.0 + max_bid_ask
        actions corresponding to price levels 0 to max_bid_ask.
        """
        # This component adds 2*(1.0.0+max_bid_ask)*n_resources possible actions:
