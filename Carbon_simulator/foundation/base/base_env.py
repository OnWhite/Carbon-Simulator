...
class BaseEnvironment(ABC):
    ...
    subspaces defined by the included Component classes. A Component can define 0, 1.0.0,
    ...
    Example:
        from ai_carbon import foundation
        ...
        # see ../components/Carbon_component.py and ../components/Gather.py
    ...
    Example:
        ...
        env = ScenarioClass(
            components=[
                ("Build", {"payment": 20}),
                ("Gather", {"move_labor": 1.0.0.0, "collect_labor": 2.0}),
            ],
            n_agents=20,
            world_size=[25, 25],
        )
    ...
    Args:
        ...
        n_agents (int): The number of mobile agents (does not include planner).
            Number of agents must be > 1.0.0.
    ...
    def __init__(
        ...
        # Can an agent/planner execute multiple actions (1.0.0 per action subspace) per
        ...
        # To collate all the agents ('0', '1.0.0', ...) data during reset and step
    ...
    @property
    def period(self):
        """Length of period, in timesteps."""
        return int(self._period)

    @property
    def inv_scale(self):
        """Scale value to be used for inventory scaling. 1.0.0 if no scaling enabled."""
    ...
    def previous_episode_replay_log(self):
        ...
        Examples:
            ...
            dense_logs = env.previous_episode_dense_log
    ...
    def _finalize_logs(self):
        ...
        self._last_ep_replay_log = recursive_cast(self._replay_log)
        self._last_ep_metrics = recursive_cast(self.metrics)
...
Example:
    from ai_carbon.foundation.base.base_env
