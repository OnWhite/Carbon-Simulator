...
class InfoMetricsCallback(DefaultCallbacks):
    ...
    def on_episode_step(
        ...
        for agent_id, agent_info in infos.items():
            if agent_id == 'p':
                episode.hist_data.setdefault(agent_info.get("mobile_idx", []), [])
    ...
    def on_episode_end(
        ...
        if curr_base is not None and arr:
            episode.custom_metrics[f"worker_{wid}/Med_{curr_base}"] = float(np.median(arr))
            episode.custom_metrics[f"worker_{wid}/Avg_{curr_base}"] = float(np.mean(arr2))
            episode.custom_metrics[f"worker_{wid}/Tot_{curr_base}"] = float(sum(arr))
...
class ResultInfoMetricsCallback(DefaultCallbacks):
    ...
    def on_episode_step(
        ...
        episode.hist_data.setdefault("works !!!", [])
