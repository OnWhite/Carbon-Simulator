...
def train_with_sweep():
    ...
    run_config["env"]["alpha"] = wandb.config.alpha
    run_config["env"]["beta"] = wandb.config.beta

    # Build the trainer with the updated configuration
    trainer = build_trainer(run_config)

    # Train the model
    for _ in range(run_config["general"]["episodes"]):
        result = trainer.train()
    ...
    try:
        ...
        if True:
            ...
            sweep_config = {
                "method": "grid",
                "parameters": {
                    "alpha": {"values": [50, 100, 150]},
                    "beta": {"values": [10, 50, 100]},
                },
            }

            sweep_id = wandb.sweep(sweep_config, project="taxation")
            ...
            wandb.init(
                ...
                name="tax-16-2-8000-50000-littleEpisodes",
                config=run_config,
